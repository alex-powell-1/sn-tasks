...
class Product:
    ...
    def get_top_child_product(self):
        ...
        children = create_top_items_report(
            beginning_date=dates.one_year_ago,
            ending_date=dates.last_year_forecast,
            merged=True,
...
def get_qty_sold_all_items():
    ...
    (VI_PS_TKT_HIST.POST_DAT >= ''2020-01-01'') and (VI_PS_TKT_HIST.POST_DAT <= ''{date_presets.Dates().today}'')', ' 
...
def set_products_to_inactive(eh=error_handler):
    count = 0
...
if __name__ == '__main__':
    print(get_all_back_in_stock_items(date_presets.Dates().one_year_ago))
